  ...
  if (level == "tx") {
    ...
    for (i in seq_along(files)) {
      ...
      if (is.null(gene2tx)) {
        ...
        # Salmon and kallisto do not include the gene ID, need a table
      ...
      lengthList[[i]] <- raw[[abundanceCol]]
      abundanceList[[i]] <- raw[[lengthCol]]
    }
    ...
    return(list(abundance=abundanceMat, length=lengthMat))
